...
function printCodeHighlight(d) {
    ...
    if (line > 0 && logger_1.Logger.meaningfulLine(srcLines[line - 1])) {
        ...
        if (logger_1.Logger.LEFT_PADDING.length + beforeLine.text.length > logger_1.Logger.MAX_LEN) {
            beforeLine.text = beforeLine.text.substr(0, logger_1.Logger.MAX_LEN - logger_1.Logger.LEFT_PADDING.length - 1);
        }
    ...
    if (line + 1 < srcLines.length && logger_1.Logger.meaningfulLine(srcLines[line + 1])) {
        ...
        if (logger_1.Logger.LEFT_PADDING.length + afterLine.text.length > logger_1.Logger.MAX_LEN) {
            afterLine.text = afterLine.text.substr(0, logger_1.Logger.MAX_LEN - logger_1.Logger.LEFT_PADDING.length - 1);
        }
