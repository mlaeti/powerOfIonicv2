...
function sass(context, configFile) {
    ...
    return worker_client_1.runWorker('sass', context, configFile)
        .then(function () {
...
exports.sass = sass;
function sassUpdate(event, path, context) {
...
function sassWorker(context, configFile) {
    return new Promise(function (resolve, reject) {
        if (!context.moduleFiles) {
            ...
            context.moduleFiles = rollup_1.getModulePathsCache();
            if (!context.moduleFiles) {
                reject(new logger_1.BuildError('Cannot generate Sass files without first bundling JavaScript ' +
                    'files in order to know all used modules. Please build JS files first.'));
                return;
            }
        }
        var sassConfig = config_1.fillConfigDefaults(configFile, taskInfo.defaultConfigFile);
...
function render(context, sassConfig) {
    return new Promise(function (resolve, reject) {
        ...
        node_sass_1.render(sassConfig, function (renderErr, sassResult) {
            if (renderErr) {
                // sass render error!
                if (renderErr.file) {
                    var sassFile = renderErr.file;
                    ...
                    logger_1.Logger.error("sass: " + sassFile + ", line: " + renderErr.line);
                    logger_1.Logger.log(renderErr.message);
                    console.log(''); // just for a new line
                }
                else {
                    logger_1.Logger.error("sass error: " + renderErr);
                }
                reject();
            }
            else {
...
function writeOutput(sassConfig, cssOutput, mappingsOutput) {
    return new Promise(function (resolve, reject) {
        ...
        fs_extra_1.writeFile(sassConfig.outFile, cssOutput, function (cssWriteErr) {
            if (cssWriteErr) {
                reject(new logger_1.BuildError("Error writing css file, " + sassConfig.outFile + ": " + cssWriteErr));
            }
            else {
